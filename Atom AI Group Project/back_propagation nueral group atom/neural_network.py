...
class Network:
    ...
    def foward(self):
        ...
        for epoch in self.input:
            ...
            layer1 = sigmoid_curve(np.dot(self.weights1, epoch))
            layer2 = sigmoid_curve(np.dot(self.weights2, layer1))
    ...
    def back_propagation(self, input, target):
        ...
        output_vector_hidden = sigmoid_curve(output_vector1)

        output_vector2 = np.dot(
            self.weights2, output_vector_hidden)
        output_vector_network = sigmoid_curve(output_vector2)
